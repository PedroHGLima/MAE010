...
static void Get_input(int my_rank, int comm_sz, double *a_p, double *b_p,
                      int *n_p)
{
    if (my_rank == 0)
    {
...
int main(void)
{
    ...
    double a, b, h, local_a, local_b, local_integral, total_integral = 0.;
    double start_time, finish_time, local_elapsed, elapsed;
    ...
    else
    {
        ...
        {
            MPI_Recv(&local_integral, 1, MPI_DOUBLE, src, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            total_integral += local_integral;
        }
    }
